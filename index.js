...
module.exports = function LetMeTarget(mod) {
	...
	mod.command.add("rod", {
		...
		"$none": () => {
			...
			mod.command.message(`Rod ${mod.settings.autoHeal ? "enabled" : "disabled"}`);
		}
	});

	mod.game.me.on("change_zone", () => {
		if (!mod.game.me.inOpenWorld && mod.game.me.class === "priest") {		
			mod.command.message(`Module ${mod.settings.autoHeal ? "<font color='#00ff00'>enabled</font>" : "<font color='#ff0000'>disabled</font>"}`);
		}
	});
